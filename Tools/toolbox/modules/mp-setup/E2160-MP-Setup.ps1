...
if (-not $needsRegUpdate) {
    ...
    if (Test-Path $backupBase) {
        $backupFile = Join-Path $desktopPath "E2160-Backup-$timestamp.reg"
...
$fwRules = @(
    @{ Name = 'E2160 - Server Port (TCP 17171)'; Protocol = 'TCP'; LocalPort = '17171'; RemoteAddress = $Subnet },
    @{ Name = 'E2160 - Game Port (UDP 17771)';     Protocol = 'UDP'; LocalPort = '17771';   RemoteAddress = $Subnet },
    @{ Name = 'E2160 - ICMPv4 Allow Subnet';             Protocol = 'ICMPv4'; RemoteAddress = $Subnet }
)

function Set-E2160FirewallRules {
    param($rules)
    foreach ($r in $rules) {
...
if (-not $needsFwUpdate) {
    ...
    try {
        Set-E2160FirewallRules -rules $fwRules
